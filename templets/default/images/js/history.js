var $MH={
	...
	piclist: function (){
		...
		for(var i=0;i<a.video.length;i++){
			...
			img_li += "<li class=\"pic\"><a href=\"" + a.video[i].link + "\" target=\"_self\"><img width=\"90\" height=\"120\" src=\"" + a.video[b].pic + "\" alt=\"" + a.video[i].name + "\" border=\"0\"/></a>\
	...
	recordHistory: function(video){
		...
		var a = $MH.getCookie('HISTORY'), b = new Array(), c = 3;
		if(a !='' && a != null && a != 'null'){
			a = $MH.tryjosn(a);
			for(var i=0;i<a.video.length;i++){
				if(c>$MH.limit){break;}
